...
- [Class <-> Identity context](https://github.com/tambetvali/Landscopes/blob/main/Landscapes/exterior.md) - this text shows that mathematically, (class <-> instance) <---> (infinity <-> identity):
  - Identity syntax implies common class syntax.
  - Infinity <-> Identity contain the same underlying sets as Class <-> Instance; thus, same language can be mapped.

Why it gets accelerating?
- Take slice \[0; 1\] inclusive.
- You can see it contains full length of slice, plus 1 point: "1" itself, and "0" itself, are two *infinitesimals* where there should be one, to not accelerate.
- One infinitesimal added to infinity defines acceleration, a direction to next infinity - the last item is *limit value*, not contained in the class.
- For example, from -1 to 1, in both directions, we can see we cannot logically map the whole without adding one infinitesimal - this break the logic; thus, the whole is accelerated.
- For this reason, we say Class = exp(Instance) or Instance = log(class): the properties, which appear in infinities, are same named but exponent.
  - For example, lets say R is infinite set of r's, and each r is moving 1 mp/h; thus R itself is moving 1 mp/h, but on exponential scale!
